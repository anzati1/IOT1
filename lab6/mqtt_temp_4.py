...
def handle_command(client, userdata, message):
    ...
    try:
        ...
        if 'led_on' in payload:
            if payload['led_on']:
                led.on()
                print("LED turned ON by command")
            else:
                led.off()
                print("LED turned OFF by command")
    ...
    try:
        while True:
            ...
            if celsius is not None:
                ...
                telemetry = {
                    'temperature': celsius,  # Keep original temperature key for compatibility
                ...
                # Note: LED control is now handled by commands from the server
                # The server will send commands based on the temperature threshold
            else:
